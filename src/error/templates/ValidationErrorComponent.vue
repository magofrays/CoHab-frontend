<script setup lang="ts">
import type {ValidationError} from "@/error/types/errors.ts";

defineProps<{
  error: ValidationError | null;
}>()
</script>

<template>
  <div v-if="error" class="flex items-start">
    <svg class="w-5 h-5 text-red-500 mt-0.5 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
      <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"></path>
    </svg>
    <div>
      <h3 class="text-sm font-medium text-red-800">{{error.message}}</h3>
      <div v-for="fieldError in error.fieldErrors" :key="fieldError.field">
        <h5 class="text-sm font-medium text-red-800">Ошибка в поле {{fieldError.field}}: {{fieldError.message}}</h5>
      </div>
      <p class="text-sm text-red-700 mt-1"></p>
    </div>
  </div>
</template>

<style scoped>

</style>